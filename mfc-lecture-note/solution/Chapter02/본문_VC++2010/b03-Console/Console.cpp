...
int _tmain(int argc, TCHAR* argv[], TCHAR* envp[])
{
	...
	if (hModule != NULL)
	{
		...
		else
		{
			...
			mapData[_T("홍길동")] = new PHYSIQUE(172, 72);
			mapData[_T("박철수")] = new PHYSIQUE(164, 67);
			mapData[_T("오영희")] = new PHYSIQUE(166, 51);
			mapData[_T("구오성")] = new PHYSIQUE(182, 80);
			mapData[_T("김순희")] = new PHYSIQUE(159, 48);
			mapData[_T("최성주")] = new PHYSIQUE(177, 69);
			mapData[_T("신여정")] = new PHYSIQUE(172, 51);
			...
			if ( mapData.Lookup(strName, pData) == TRUE )
			{
				...
				_tprintf(_T("%s의 키: %d, 몸무게: %d\n"),
					strName, pphysique->uiHeight, pphysique->uiWeight);
			}
			else
